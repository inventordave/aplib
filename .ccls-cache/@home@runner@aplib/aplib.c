...
AP PowAP( AP A )	{
	...
	// AP version of <math.h> pow() function 
	// Interim solution:

	#include <math.h>
	double r = pow( EULER_GAMMA_NATIVE, (double)str2int(A->integer) );

	AP C = NewAP( 0,0 );

	C->integer = getstring( int2str((int)r) );
	return C;
}
...
AP NLOG(AP x) { return NLOGP(x, DefaultPrecision); }


char* DIV_str( char* N, char* M )	{
...
char* NLOG_str( char* _ )	{
	...
	char* r;
	r = getstring( C->integer );

	if( C->fractional )	{

		strcat( r, "." );
		strcat( r, C->fractional );
...
AP NLOGP(AP x, AP epsilon) {
  ...
  do {
    ...
    // pow(v) should be in c stdlib, calculates the value of Euler's constant
    // (irrational, approx 2.71828) raised to power v. PowAP(v) is an AP derivation of pow(v) in math.h
...
// log_b(x) = ln(x)/ln(b).
AP LOGb_(AP A, unsigned int base) {
...
char *ConvertBase(
    char *A, int prev_base,
    int new_base); // This stub is also to remind me to do alt. implementations.
// A lot of calls to operators are not going to require full AP structures, just
// digitstrings.
...
//
// where [OP_CLASS]_xy naming convention indicates an input of a rational x/y,
// in form of 2 parameters notionally called x and y.
//
// An alt macro for LOGb as so:
// #define alt_LOGb( x,b ) ( { AP xt=(NLOG(x));AP bt=(NLOG(b)); AP
// rt=(DIV(xt,bt)); FreeAP(xt);FreeAP(bt); (rt); } ) NOTE TO SELF: I need to
...
L setPartW(AP A, char *_) { return setPart(A, _, PartW); }
L setPartF(AP A, char *_) { return setPart(A, _, PartF); }
...
AP ADDP(AP A, AP B, AP P) {
  ...
  2. If the signs are different, subtract the numbers and take the sign of the
  bigger* number.
  */
...
#define CONCAT(__A, __B) strcat(__A, __B)
    ...
    {
      report( "Remainder = %s\nDivisor = %s\nLOOPS = %d\nC = %s.%s\n", Remainder->integer, \
        B->integer ,LOOPS, C->integer, C->fractional );
...
// The function below (DEC_2_BIN) is not finished. I forgot about it, because I
// keep getting drunk. It's fun programming when you're drunk, but you always
// forget what you were working on!!
char *DEC_2_BIN(char *input, L packed) {
  /**
  PArAm "int packed" is A flag to determine if the returned binary string should
  be rounded in length to A multiple of 8. In other words, 127 would be
  "01111111" insteAd of "1111111". An Argument of 0 meAns "do not round", A
  non-0 value meAns "round up string-length to multiple of 8".
  */
  ...
  while (flag) {
    ...
    for (i = 0; i < length; i++) {
      ...
      // if it is, the remainder is noted as a binary digit 1, And the remainder
      // itself discarded.
...
loop:
  if (bin_string[i] == '1') {
    // only necessary because of bit-strings with leading '0's, which is not
    // always true for return values of DEC_2_BIN()
    int k = i;
...
// DIGIT-WISE FNCS
char d(AP A, L index) { return A->integer[index]; }
...
void setDigit(AP A, L index, char d) { A->integer[index] = d; }
...
#
  ...
  /*// ARCHAIC SUBSECTION FOR TRIMMING LEADING ZERO'S BY SHUNTING THE POINTER
  FROM THE FRONT. int len = i; for( i=0; i<len; i++ ) if( _.integer[i]=='0' )
  {
...
// SIGN FNCS
char getSign(AP A) { return A->sign; }
...
signed int OverFlow(AP C, int result, signed k) {
  ...
  return k - 1;
  // remember, overflow can be from 1 to 8! If [k-1] >= 2, itself may additively
  // overflow to [k-2];
}
