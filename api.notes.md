...
Perhaps a compilation stack layer invoked after successful build of a compilation invocation, that after the codebase proves to succesfully compile, might run a "lint", or "pretty-print" pass over the source files. The pre-transformed source file strings could be preserved using the standard versioning system of git.
